...
const navItems = [
  { href: '/', label: 'Overview' },
  { href: '/control-panel', label: 'Capabilities' },
  ...
  return (
    <footer className="site-footer">
      <div className="container-wide footer-grid">
        ...
        <div><div className="footer-heading">Connect</div><div className="footer-links"><a href="mailto:projects@orbitcontrols.com" data-testid="link-footer-email">projects@orbitcontrols.com</a><a href="tel:+13125550184" data-testid="link-footer-phone">+1 312 555 0184</a><span className="muted">North America / Central Time</span></div></div>
      </div>
      <div className="container-wide footer-bottom"><span>© 2025 ORBIT CONTROLS LLC</span><span>UL 508A SHOP · BUILT IN THE USA</span></div>
    </footer>
  );
}
...
function Stats() {
  return <div className="stats-grid" data-testid="stats-grid">
    <div className="stat"><div className="stat-value">17<span style={{ color: 'hsl(var(--primary))' }}>+</span></div><div className="stat-label">Years in the panel shop</div></div>
    <div className="stat"><div className="stat-value">4.8k</div><div className="stat-label">Panels shipped to date</div></div>
    <div className="stat"><div className="stat-value">100<span style={{ color: 'hsl(var(--accent))' }}>%</span></div><div className="stat-label">Panels tested before ship</div></div>
    <div className="stat"><div className="stat-value">0.2<span style={{ color: 'hsl(var(--primary))' }}>%</span></div><div className="stat-label">Average field rework rate</div></div>
  </div>;
}

function Process({ compact = false }: { compact?: boolean }) {
  const steps = [
    ['01', 'Scope', 'We translate your drawings, I/O list, and field constraints into a buildable panel plan.'],
    ['02', 'Engineer', 'Our team resolves the details that make installation and commissioning predictable.'],
    ['03', 'Build', 'A disciplined shop floor assembles, labels, wires, and documents every enclosure.'],
    ['04', 'Prove', 'Continuity, functional, and visual checks happen before the panel leaves our dock.'],
  ];
...
function Home() {
  return <Shell>
    ...
    <main>
      <section className="hero"><div className="container-wide hero-grid"><div className="hero-copy"><div className="eyebrow">Custom electrical control panels / Since 2008</div><h1>Make the <em>machine</em> dependable.</h1><p className="hero-lede">Orbit Controls builds the control panels behind critical equipment. Build-to-print or engineered with you, every enclosure is wired, inspected, and tested before it reaches your floor.</p><div className="hero-actions"><Link href="/contact" className="button-primary" data-testid="link-hero-quote">Start a project <ArrowRight size={15} /></Link><Link href="/control-panel" className="button-secondary" data-testid="link-hero-capabilities">Explore capabilities <ChevronRight size={15} /></Link></div><div className="hero-note"><span className="pulse-dot" /><span><strong>Shop status:</strong> accepting Q3 production schedules</span></div></div><PanelVisual /></div></section>
      <Ticker />
...
const services = [
  ...
  { icon: <Factory size={27} />, title: 'Build-to-print', copy: 'Have a released drawing set? We turn it into a consistent, traceable build with the care and speed your production schedule demands.' },
  { icon: <Zap size={27} />, title: 'Power & motor control', copy: 'Clean integration of breakers, disconnects, starters, VFDs, soft starts, and power distribution for demanding industrial environments.' },
  { icon: <Gauge size={27} />, title: 'PLC & automation', copy: 'PLC, HMI, remote I/O, network hardware, and controls architecture assembled as one coherent, documented system.' },
